...
let sub_align_right_item='echo "align_right_item"|let col=getcurpos()[2]| exe ''''''<,''''>s/\v(\s*)(%''.col.''v)(\s*)((\S+))/\=submatch(1).submatch(4).submatch(3)/'''
let sub_align_left_item='echo "align_left_item"|let col=getcurpos()[2]| exe ''''''<,''''>s/\v((\S+))(\s*)(%''.col.''v)(\s*)/\=submatch(3).submatch(1).submatch(5)/'''
